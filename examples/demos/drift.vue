...
<div>
    ...
    <div class="scroll-row">
        <div v-for="icon in bubblesIcon" :key="icon">
            <img class="bubble" :src="icon" @click="addBubble(icon)">
        </div>
    </div>
    <vego-canvas :width="canvasWidth" :height="canvasHeight" @mousedown="noEdit">
        <container v-if="bgImage" :x="panel.x" :y="panel.y" :reg-x="panel.regX" :reg-y="panel.regY" :rotation="panel.rotation">
            <targetImage :dx="0" :dy="0" :dwidth="panel.width" :dheight="panel.height" :src="bgImage" @pressmove="move" @mousedown="prepare"></targetImage>
        </container>
        <container v-for="bb in bubbles" :key="bb.key" :x="bb.x" :y="bb.y" :reg-x="bb.regX" :reg-y="bb.regY" :rotation="bb.rotation" @mouseenter="bubbleMouseOver(true, $event)" @mouseleave="bubbleMouseOver(false, $event)">
            <targetImage :dx="0" :dy="0" :dwidth="bb.width" :dheight="bb.height" :src="bb.img" @pressmove="move" @mousedown="prepare($event, bb)"></targetImage>
        </container>
        <container :x="panel.x" :y="panel.y" :reg-x="panel.regX" :reg-y="panel.regY" :rotation="panel.rotation">
            <controlRect :x="0" :y="0" :width="panel.width" :height="panel.height"></controlRect>
            <controlPoint :dx="panel.width - 25" :dy="panel.height - 25" :dwidth="50" :dheight="50" :src="resizeIcon" @pressmove="resize" @mousedown="prepare"></controlPoint>
            <controlPoint :dx="panel.width - 25" :dy="-25" :dwidth="50" :dheight="50" :src="closeIcon" @click="deleteimg"></controlPoint>
            <controlPoint :dx="-25" :dy="panel.height - 25" :dwidth="50" :dheight="50" :src="rotateIcon" @pressmove="rotate" @mousedown="prepare"></controlPoint>
        </container>
    </vego-canvas>
...
export default {
    ...
    methods: {
        ...
        fileChanged(e) {
            const file = this.$refs.picfilereader.files[0];
            if (file) {
